            ...
            Dialogflow.Event(res.CLIENT.Dialogflow, messageText, senderID, recipientID)
                ...
                .then(function (retorno) {
                    ...
                    if (res.firstNLP.queryResult.action.length > 0) {
                        ...
                        if (action.length > 0) {
                            var _listMessages = events(action, res.CLIENT.CLIENT_ACCESS_TOKEN, senderID, res.Messages, recipientID);
                            return (_listMessages);
                        }
                        else
                            return;
            ...
            Dialogflow.detectIntent(res.CLIENT.Dialogflow, messageText, res.User, senderID, recipientID)
                ...
                .then(function (retorno) {
                    ...
                    if (res.firstNLP.queryResult.action.length > 0) {
                        ...
                        action = action.filter(x => x.indexOf('EVENTS') > -1);
                        if (action.length > 0) {
                            var _listMessages = events(action, res.CLIENT.Dialogflow.Clientaccesstoken, senderID, res.Messages, recipientID);
                            return (_listMessages);
                        }
                        else
                            return;
...
function events(action, token, senderID, messages, pageID) {
    ...
    const _list = Promise.resolve()
        .then(function () {
            return Dialogflow.Event(token, event, senderID, pageID)
                .then(function (retorno) {
                    itemNLP = retorno;
        ...
        .then(function (retorno) {
            ...
            if (action.length > 0) {
                return (events(action, token, senderID, messages, pageID)); // RECURSE!
            }
            else
                return messages;
        })
